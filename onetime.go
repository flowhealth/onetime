...
// CTOTP code calculated with the current time and the given secret and OTP value.
// Correct cloc skew +- 1 min
func (otp *OneTimePassword) COTP(secret []byte, value string) (res bool, err error) {
	ui, err := strconv.ParseUint(value, 10, 0)
	if err != nil {
		return false, err
	}
	if otp.HOTP(secret, otp.steps(time.Now())) == uint(ui) {
		return true, nil
	}
	if otp.HOTP(secret, otp.steps(time.Now())+uint64(1)) == uint(ui) {
		return true, nil
	}
	if otp.HOTP(secret, otp.steps(time.Now())-uint64(1)) == uint(ui) {
		return true, nil
